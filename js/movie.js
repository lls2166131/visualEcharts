...
const formattedRenderChart = (key, data) => {
  let d = data
  const a  = _.groupBy(data, key)
  let mapper_option = {
    area: optionForArea,
    type: optionForType,
    year: optionForYear,
  }
  const option = mapper_option[key](a)
  let maper_store = {
    area: 'pie',
    type: 'bar',
    year: 'line',
  }
  let t = maper_store[key]
  const c = chartStore[t]
  c.setOption(option)
}

const renderChart = function(d) {
    const data = d
    const types = ['area', 'type', 'year']
    for (let t of types) {
      formattedRenderChart(t, d)
    }
    // formattedRenderChart('area', d)
    // formattedRenderChart('type', d)
    // formattedRenderChart('year', d)

    // const area = _.groupBy(data, 'area')
    // log('area  in renderChart', area)
